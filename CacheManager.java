...
public class CacheManager {
    ...
    public static synchronized void put(String url, byte[] data) {
        ...
        System.out.println("taille : " + data.length);
        if (data.length > MAX_MEMORY_ITEM_SIZE) {
            System.out.println("Données trop volumineuses (plus de " + MAX_MEMORY_ITEM_SIZE + " octets), stockage uniquement dans le dossier.");
    
            File cacheFile = new File(CACHE_DIRECTORY, cacheFileName);
            try (FileOutputStream fos = new FileOutputStream(cacheFile)) {
                fos.write(data);
                System.out.println("Donnée écrite dans le fichier de cache sur disque : " + cacheFile.getAbsolutePath());
            } catch (IOException e) {
                System.err.println("Erreur lors de l'écriture dans le cache: " + e.getMessage());
            }
    
        } else {
            System.out.println("Données stockées uniquement en mémoire pour l'URL : " + url);
                memoryCache.put(cacheFileName, data);
        }
    ...
    // fun pourgénérer le nom de fichier cache à partir de l'URL
    ...
    public static void listUrlsFromMappingFile() {
        try (BufferedReader reader = new BufferedReader(new FileReader(MAPPING_FILE))) {
            String line;
            while ((line = reader.readLine()) != null) {
                int lastEqualsIndex = line.lastIndexOf("=");
                if (lastEqualsIndex != -1) {
                    String url = line.substring(0, lastEqualsIndex);
                    String cacheFileName = line.substring(lastEqualsIndex + 1);
                    System.out.println("URL : " + url);
                    System.out.println("Cache file nom : " + cacheFileName);
                } else {
                    System.err.println("Format invalide dans la ligne : " + line);
                }
            }
        } catch (IOException e) {
            System.err.println("Erreur lors de la lecture du fichier de mapping : " + e.getMessage());
        }
    }
}
